...
class Instagram
{
    ...
    private $userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36';
//    private $userAgent = 'Mozilla/5.0 (Linux; Android 8.1.0; motorola one Build/OPKS28.63-18-3; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/70.0.3538.80 Mobile Safari/537.36 Instagram 72.0.0.21.98 Android (27/8.1.0; 320dpi; 720x1362; motorola; motorola one; deen_sprout; qcom; pt_BR; 132081645)';
    ...
    public static function searchTagsByTagName($tag)
    {
        ...
        if (!isset($jsonResponse['status']) || $jsonResponse['status'] !== 'ok') {
            throw new InstagramException('Response code is not equal 200. Something went wrong. Please report issue.', 404);
        }
...
//print_r($arr);
    ...
    public function getMediaByUrl($mediaUrl)
    {
        ...
        if (static::HTTP_NOT_FOUND === $response->code) {
            throw new InstagramNotFoundException('Media with given code does not exist or account is private.', static::HTTP_NOT_FOUND);
        }
        if (static::HTTP_RATE_LIMIT === $response->code) {
            throw new InstagramRateLimitException('Rate limit exception',static::HTTP_RATE_LIMIT);
        }
        ...
        if (!isset($mediaArray['graphql']['shortcode_media'])) {
            throw new InstagramException('Media with this code does not exist', 500);
        }
    ...
    public function getTagPage($tag, $count = 12, $maxId = '', $minTimestamp = null)
    {
        ...
        while ($index < $count && $hasNextPage) {
            ...
            if ($response->code === static::HTTP_NOT_FOUND) {
                throw new InstagramNotFoundException('This tag does not exists or it has been hidden by Instagram', static::HTTP_NOT_FOUND);
            }
    ...
    public function getPaginateFollowing($accountId, $count = 20, $pageSize = 20, $delayed = true, $nextPage = '')
    {
        ...
        while (true) {
            $response = Request::get(Endpoints::getFollowingJsonLink($accountId, $pageSize, $endCursor),
                $this->generateHeaders($this->userSession));
            if ($response->code === static::HTTP_NOT_FOUND) {
                throw new InstagramNotFoundException('Account with this id doesn\'t exist', static::HTTP_NOT_FOUND);
            }
